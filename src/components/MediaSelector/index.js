...
import { getMedium, loadMedia } from '../../actions/media';
...
function MediaSelector({ value = null, onChange }) {
  ...
  const medium = useSelector((state) => {
    return state.media.items[value] || null;
  });
  ...
  if (!loading && mediumFetch && uploadedMedium) {
    ...
    const fetchedMedium = media.items[fetchedId];
  ...
  return (
    <>
      ...
      <Space direction="vertical">
        {medium ? (
          <img
            src={medium.url?.proxy + '?resize:fill:200:200/gravity:sm'}
            alt={medium.alt_text}
            width="100%"
          />
        ) : (
